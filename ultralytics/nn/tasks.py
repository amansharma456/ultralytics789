...
# Defined directly in tasks.py to avoid import caching issues
# ═══════════════════════════════════════════════════════════════════
...
class _SwinWindowAttention(torch.nn.Module):
...
class _SwinBlock(torch.nn.Module):
    def __init__(self, dim, num_heads, window_size,
        ...
        self.norm1       = torch.nn.LayerNorm(dim)
        self.norm2       = torch.nn.LayerNorm(dim)
        self.attn        = _SwinWindowAttention(dim, window_size, num_heads)
        self.mlp         = torch.nn.Sequential(
            torch.nn.Linear(dim, dim * 4),
            torch.nn.GELU(),
            torch.nn.Linear(dim * 4, dim),
        )
        self.dp_rate = drop_path_rate

    def _drop_path(self, x):
        if not self.training or self.dp_rate == 0.0:
            return x
        keep  = 1.0 - self.dp_rate
        shape = (x.shape[0],) + (1,) * (x.ndim - 1)
    ...
    def forward(self, x, H, W):
        ...
        windows     = _swin_window_partition(x, self.window_size)
        ...
        x           = _swin_window_reverse(attn_out, self.window_size, Hp, Wp)
...
class _SwinPatchMerging(torch.nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.norm      = torch.nn.LayerNorm(4 * dim)
        self.reduction = torch.nn.Linear(4 * dim, 2 * dim, bias=False)
...
class SwinBackbone(torch.nn.Module):
    """
    Swin Transformer Tiny backbone for YOLOv8.
    ...
    Returns [P2, P3, P4, P5] as (B, C, H, W) tensors.
    Channel widths [96, 192, 384, 768] match neck expectations.
    """
    ...
    def __init__(self, in_chans=3, drop_path_rate=0.1):
        super().__init__()
        dims, depths, heads, ws = (self._DIMS, self._DEPTHS,
                                   self._HEADS, self._WINDOW)
        self.dims = list(dims)

        # patch embed: 4×4 conv stride 4 → P2 resolution
        ...
        self.out_norms = torch.nn.ModuleList(
            [torch.nn.LayerNorm(d) for d in dims])

        for i in range(4):
            # patch merging before stages 1,2,3 (not stage 0)
            if i > 0:
                self.merges.append(_SwinPatchMerging(dims[i - 1]))
            else:
                self.merges.append(torch.nn.Identity())

            stage = torch.nn.ModuleList([
                _SwinBlock(
                    dim        = dims[i],
                    num_heads  = heads[i],
                    window_size= ws,
                    shift_size = 0 if b % 2 == 0 else ws // 2,
                    drop_path_rate = dpr[cur + b]
                )
    ...
    def forward(self, x):
        ...
        outs = []
        for i in range(4):
            # merge patches (downsample) before stages 1,2,3
            if i > 0:
                x = self.merges[i](x, H, W)
                H, W = H // 2, W // 2
            ...
            # convert tokens → spatial feature map and save
            x_out = self.out_norms[i](x)
...
class _Swish(torch.nn.Module):
    def forward(self, x):
        return x * torch.sigmoid(x)


class _SEBlock(torch.nn.Module):
    """Squeeze-and-Excitation block used inside MBConv."""
    def __init__(self, in_ch, se_ch):
        super().__init__()
        self.se = torch.nn.Sequential(
            torch.nn.AdaptiveAvgPool2d(1),
            torch.nn.Conv2d(in_ch, se_ch, 1, bias=True),
            _Swish(),
            torch.nn.Conv2d(se_ch, in_ch, 1, bias=True),
            torch.nn.Sigmoid(),
        )
...
class _MBConv(torch.nn.Module):
    ...
    def __init__(self, in_ch, out_ch, kernel_size,
        ...
        if expand_ratio != 1:
            layers += [
                torch.nn.Conv2d(in_ch, mid_ch, 1, bias=False),
                torch.nn.BatchNorm2d(mid_ch, eps=1e-3, momentum=0.01),
                _Swish(),
            ]
        # Depthwise conv
        layers += [
            torch.nn.Conv2d(mid_ch, mid_ch, kernel_size, stride=stride,
        ...
        # SE
        layers.append(_SEBlock(mid_ch, se_ch))
        # Pointwise projection
        layers += [
            torch.nn.Conv2d(mid_ch, out_ch, 1, bias=False),
            torch.nn.BatchNorm2d(out_ch, eps=1e-3, momentum=0.01),
        ]
...
class EfficientNetB0Backbone(torch.nn.Module):
    ...
    EfficientNet-B0 stage config:
        stage | repeat | kernel | stride | in_ch | out_ch | expand
          0   |   1    |   3    |   1    |   32  |   16   |   1
          1   |   2    |   3    |   2    |   16  |   24   |   6   → P2
          2   |   2    |   5    |   2    |   24  |   40   |   6   → P3
          3   |   3    |   3    |   2    |   40  |   80   |   6
    ...
    _STAGE_CFG = [
        (1, 3, 1,  32,  16, 1),
        (2, 3, 2,  16,  24, 6),   # → P2  24ch  stride 4
        (2, 5, 2,  24,  40, 6),   # → P3  40ch  stride 8
        (3, 3, 2,  40,  80, 6),
        (3, 5, 1,  80, 112, 6),   # → P4  112ch stride 16
        (4, 5, 2, 112, 192, 6),
        (1, 3, 1, 192, 320, 6),   # → P5  320ch stride 32
    ]

    # which stage index produces each scale
    _P2_STAGE = 1
    _P3_STAGE = 2
    _P4_STAGE = 4
    _P5_STAGE = 6
    ...
    def __init__(self, in_chans=3, drop_path_rate=0.2):
        ...
        # output channel sizes exposed for parse_model
        self.dims = [24, 40, 112, 320]   # P2, P3, P4, P5

        # Stem: 3x3 conv stride 2
        self.stem = torch.nn.Sequential(
            torch.nn.Conv2d(in_chans, 32, 3, stride=2,
                            padding=1, bias=False),
            torch.nn.BatchNorm2d(32, eps=1e-3, momentum=0.01),
            _Swish(),
        )

        # Build all 7 stages
        total_blocks = sum(cfg[0] for cfg in self._STAGE_CFG)
        dpr = torch.linspace(0, drop_path_rate, total_blocks).tolist()
        cur = 0

        self.stages = torch.nn.ModuleList()
        for repeat, kernel, stride, in_ch, out_ch, expand in self._STAGE_CFG:
            blocks = []
            for i in range(repeat):
                blocks.append(_MBConv(
    ...
    def forward(self, x):
        """Returns [P2_24ch, P3_40ch, P4_112ch, P5_320ch]."""
        x = self.stem(x)
        ...
        for i, stage in enumerate(self.stages):
            x = stage(x)
            if i == self._P2_STAGE:
                outs.append(x)   # P2 24ch
            elif i == self._P3_STAGE:
                outs.append(x)   # P3 40ch
            elif i == self._P4_STAGE:
                outs.append(x)   # P4 112ch
            elif i == self._P5_STAGE:
                outs.append(x)   # P5 320ch
